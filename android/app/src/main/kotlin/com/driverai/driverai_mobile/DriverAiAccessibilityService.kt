...
class DriverAiAccessibilityService : AccessibilityService() {
    ...
    try {
        ...
        Log.e(
            "DriverAI_OCR",
            "Error ejecutando OCR: ${e.message}"
        )
...
class DriverAiAccessibilityService : AccessibilityService() {
    ...
    private fun captureScreenWithOcr() {
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.R) {
            Log.e(
                "DriverAI_OCR",
                "takeScreenshot requiere Android 11+"
            )
            return
        }
...
runOcr(softwareBitmap)
                        ...
                        Log.e(
                            "DriverAI_OCR",
                            "Error preparando screenshot: ${e.message}"
                        )
                        isProcessingOcr = false
                    }
                }
                ...
                override fun onFailure(errorCode: Int) {
                    Log.e(
                        "DriverAI_OCR",
                        "Error takeScreenshot code=$errorCode"
                    )
                    isProcessingOcr = false
                }
            }
        ...
        recognizer.process(image)
            .addOnSuccessListener { visionText ->
                ...
                if (offer == null) {
                    Log.d(
                        "DriverAI_OCR",
                        "OCR leído, pero no parece oferta válida"
                    )
                    isProcessingOcr = false
                    return@addOnSuccessListener
                }
    ...
    private fun parseOffer(text: String): OfferResult? {
        ...
        Log.d(
            "DriverAI_OCR",
            "Parse => fare=$fare totalKm=$totalKm totalMin=$totalMinutes pickupKm=$pickupKm tripKm=$tripKm"
        )
    ...
    private fun showNativeOverlay(offer: OfferResult) {
        ...
        layout.addView(
            label(
                "₡${"%.2f".format(offer.profitPerKm)} / km",
                25f,
                decisionColor
            )
        )
        ...
        layout.addView(
            label(
                "🚗 ${"%.1f".format(offer.totalKm)} km  |  ⏱ ${offer.totalMinutes} min",
                14f,
                Color.WHITE
            )
        )
        ...
        layout.addView(
            label(
                "💰 ₡${"%.0f".format(offer.fare)}  |  📈 ₡${"%.0f".format(offer.netProfit)}",
                13f,
                Color.WHITE
            )
        )
        ...
        try {
            ...
            Log.e(
                "DriverAI_OCR",
                "Error mostrando overlay: ${e.message}"
            )
        }
