...
final sharedPrefsDataSourceProvider = Provider((ref) => SharedPreferencesDataSource());
final userPreferencesRepositoryProvider = Provider<UserPreferencesRepository>((ref) {
  return UserPreferencesRepositoryImpl(ref.read(sharedPrefsDataSourceProvider));
});
...
class UserParametersNotifier extends StateNotifier<UserParameters> {
  ...
  void updateFixedCostPerTrip(double value) {
    state = state.copyWith(fixedCostPerTrip: value);
    _repository.saveParameters(state);
  }
...
final userParametersProvider = StateNotifierProvider<UserParametersNotifier, UserParameters>((ref) {
